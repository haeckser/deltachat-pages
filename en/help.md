...
- Instead of installing Delta Chat directly via APK file, you can first install the F-Froid store via [F-Droid-apk](https://f-droid.org/FDroid.apk) first and then install Delta Chat from the store. The great benefit is the information about updates and the migration of existing config and chat data.
